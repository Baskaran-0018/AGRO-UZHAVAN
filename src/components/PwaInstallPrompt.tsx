      ...
      {!isOnline && (
        <div className="bg-amber-600 text-white px-3 py-2 text-xs font-semibold flex items-center justify-between shadow-md z-50 fixed top-0 left-0 right-0 animate-in slide-in-from-top duration-300">
          <div className="flex items-center gap-2 max-w-4xl mx-auto w-full justify-center text-center">
            ...
            <span>
              <strong>{t.offlineMode || 'Offline Mode Active'}:</strong> {t.offlineDescription || 'Operating without internet. Farm records, weather advisories & ML calculations remain fully accessible.'}
            </span>
          </div>
        </div>
      )}
      ...
      {hasUpdate && (
        <div className="fixed bottom-20 md:bottom-6 right-4 z-50 max-w-sm bg-slate-900 text-white border border-emerald-500/50 rounded-2xl p-4 shadow-2xl backdrop-blur-xl flex items-start gap-3 animate-in slide-in-from-bottom-5">
          ...
          <div className="flex-1 min-w-0">
            <h4 className="text-xs font-bold text-slate-100">{t.updateAvailable || 'Update Available'}</h4>
            <p className="text-[11px] text-slate-400 mt-0.5">
              {t.updateAvailableDesc || 'A new version of AGRO AI with enhanced models is ready.'}
            </p>
            <button
              onClick={updateApp}
          ...
          <div className="relative w-full max-w-md bg-slate-900 border-t sm:border border-emerald-500/30 rounded-t-3xl sm:rounded-3xl p-5 sm:p-6 shadow-2xl text-white space-y-4 max-h-[92vh] overflow-y-auto animate-in slide-in-from-bottom-6 duration-300">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="px-1.5 py-0.2 rounded bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-[9px] font-bold uppercase tracking-wider">
                    {t.officialApp || 'Official App'}
                  </span>
                </div>
                <p className="text-xs text-emerald-400 font-medium truncate">
                  {t.intelligentAgriculture || 'Intelligent Agriculture Platform'}
                </p>
                <div className="flex items-center gap-2 mt-1 text-[11px] text-amber-400 font-bold">
                  ...
                  <span className="text-slate-300 text-[10px]">4.9 • {t.freeOffline || '100% Free & Offline'}</span>
                </div>
              </div>
            </div>
            ...
            {installedSuccess ? (
              <div className="p-4 rounded-2xl bg-emerald-950/70 border border-emerald-500/50 text-center space-y-2 animate-in zoom-in-95">
                ...
                <h4 className="font-extrabold text-sm text-white">{t.appInstalledSuccess || 'App Installed Successfully!'}</h4>
                <p className="text-xs text-emerald-300">
                  {t.installedDesc || 'You can now launch Agro Uzhavan directly from your mobile home screen.'}
                </p>
              </div>
            ) : (
              <>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="p-2.5 rounded-xl bg-slate-800/80 border border-slate-700/60 flex items-start gap-2">
                    ...
                    <div>
                      <strong className="text-slate-100 block text-[11px]">{t.instantLaunch || 'Instant Launch'}</strong>
                      <span className="text-slate-400 text-[10px]">{t.zeroLoadingTime || 'Zero loading time'}</span>
                    </div>
                  </div>
                  <div className="p-2.5 rounded-xl bg-slate-800/80 border border-slate-700/60 flex items-start gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
                    <div>
                      <strong className="text-slate-100 block text-[11px]">{t.worksOffline || 'Works Offline'}</strong>
                      <span className="text-slate-400 text-[10px]">{t.useAnywhereInField || 'Use anywhere in field'}</span>
                    </div>
                  </div>
                  <div className="p-2.5 rounded-xl bg-slate-800/80 border border-slate-700/60 flex items-start gap-2">
                    <Sparkles className="w-4 h-4 text-teal-400 shrink-0 mt-0.5" />
                    <div>
                      <strong className="text-slate-100 block text-[11px]">{t.aiLeafDoctor || 'AI Leaf Doctor'}</strong>
                      <span className="text-slate-400 text-[10px]">{t.cameraVisionScan || 'Camera vision scan'}</span>
                    </div>
                  </div>
                  <div className="p-2.5 rounded-xl bg-slate-800/80 border border-slate-700/60 flex items-start gap-2">
                    <ShieldCheck className="w-4 h-4 text-blue-400 shrink-0 mt-0.5" />
                    <div>
                      <strong className="text-slate-100 block text-[11px]">{t.zeroMbStorage || '0 MB Storage'}</strong>
                      <span className="text-slate-400 text-[10px]">{t.noAppStoreHassle || 'No app store hassle'}</span>
                    </div>
                  </div>
                </div>
                ...
                {platform === 'ios' ? (
                  <div className="p-3.5 rounded-2xl bg-slate-950/80 border border-slate-800 space-y-2.5">
                    <h4 className="text-[11px] font-extrabold text-emerald-400 uppercase tracking-wider flex items-center gap-1.5">
                      <span>{t.howToInstallIos || 'How to Install on iPhone / iPad'}</span>
                    </h4>
                    <ol className="space-y-2 text-xs text-slate-300">
                      <li className="flex items-center gap-2">
                        ...
                        <span>
                          {t.iosStep1 || 'Tap the Share icon at the bottom of Safari.'}
                        </span>
                      </li>
                      <li className="flex items-center gap-2">
                        ...
                        <span>
                          {t.iosStep2 || 'Scroll down and tap Add to Home Screen.'}
                        </span>
                      </li>
                      <li className="flex items-center gap-2">
                        ...
                        <span>
                          {t.iosStep3 || 'Tap Add on the top right. Done!'}
                        </span>
                      </li>
                    </ol>
                  </div>
                  ...
                  <div className="pt-1">
                    ...
                    >
                      ...
                      <span>{t.installAppOnDevice || 'Install App on Device (1-Tap)'}</span>
                    </button>
                  </div>
                )}
              </>
            ...
            <div className="text-center pt-1">
              ...
              >
                {t.continueInBrowser || 'Continue in browser'}
              </button>
            </div>
          </div>
        </div>
