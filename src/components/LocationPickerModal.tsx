...
import { getLocalizedLocation, translateText } from '../lib/universalTranslator';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/80 backdrop-blur-sm animate-fadeIn">
      <div className="w-full max-w-lg rounded-2xl bg-white border border-emerald-100 shadow-2xl p-5 sm:p-6 relative text-slate-900 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h2 className="text-lg font-black text-slate-900">{translateText('Set Exact Farm Location', lang)}</h2>
            <p className="text-xs text-slate-500">{translateText('Auto-detect GPS coordinates or search your village/district', lang)}</p>
          </div>
        </div>
        ...
        <div className="mb-4 p-3.5 rounded-xl bg-emerald-50 border border-emerald-200 flex items-center justify-between gap-2">
          <div className="min-w-0">
            <span className="text-[10px] font-extrabold uppercase tracking-wider text-emerald-800 block">{translateText('Current Location', lang)}</span>
            <span className="text-xs sm:text-sm font-bold text-slate-800 truncate block">{getLocalizedLocation(currentLocationName, lang)}</span>
        ...
        >
          ...
          <span>{isDetectingGps ? translateText('Detecting Live GPS Coordinates...', lang) : translateText('📍 Auto-Detect My Live GPS Location', lang)}</span>
        </button>

        {/* Divider */}
        <div className="relative flex py-2 items-center mb-4">
          <div className="flex-grow border-t border-slate-200"></div>
          <span className="flex-shrink mx-3 text-slate-400 text-xs font-semibold uppercase tracking-wider">{translateText('or search place', lang)}</span>
          <div className="flex-grow border-t border-slate-200"></div>
        </div>
        ...
        <form onSubmit={handleSearch} className="flex gap-2 mb-3">
          ...
          >
            {isSearching ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : (t.search || 'Search')}
          </button>
        </form>
        ...
        {results.length > 0 && (
          <div className="mb-4">
            <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5 block">{translateText('Search Results', lang)}</span>
              ...
              {results.map((r) => (
                ...
                >
                  <div className="min-w-0 pr-2">
                    <span className="font-bold text-slate-800 block truncate">{getLocalizedLocation(r.name, lang)}</span>
                    <span className="text-[11px] text-slate-500 truncate block">
                      {[r.admin2, r.admin1, r.country].filter(Boolean).map(x => getLocalizedLocation(x, lang)).join(', ')}
                    </span>
                  </div>
        ...
        <div>
          <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2 block">{translateText('Quick Select Region', lang)}</span>
          <div className="flex flex-wrap gap-1.5 max-h-36 overflow-y-auto">
            {POPULAR_LOCATIONS.map((preset) => {
              ...
              return (
                ...
                >
                  {getLocalizedLocation(preset.name, lang)}
                </button>
              );
            })}
          </div>
